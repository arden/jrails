...
public class CnToSpell {
    ...
    public static String getPinyinOneOnUppercase(String str) {
        String ss = "";
        for (int i = 0; i < str.length(); i++) {
            String s = str.substring(i, i + 1);
            if (s.matches("[\u4e00-\u9fa5]+")) {
                String panying = getPinyin(s);
                ss += panying.length() == 1 ? panying.toUpperCase() : panying.substring(0,1).toUpperCase() + panying.substring(1,panying.length()) ;
            } else {
                ss += s;
            }
